...
namespace ECM2
{
  ...
  public class Character : MonoBehaviour
  {
    ...
    [SerializeField] private MovementMode _startingMovementMode;
    ...
    [SerializeField] private float _minAnalogWalkSpeed;
    ...
    [SerializeField] private float _brakingDecelerationWalking = 20f;
    ...
    [Tooltip("Friction to apply to lateral movement when falling. \n" +
             "If useSeparateBrakingFriction is false, also affects the ability to stop more quickly when braking (whenever acceleration is zero).")]
    [SerializeField] private float _fallingLateralFriction = 0.3f;
    ...
    [SerializeField] private bool _canEverJump;

    [Tooltip("Can jump while crouching ?")]
    [SerializeField] private bool _canJumpWhileCrouching;
    ...
    [SerializeField] private float _jumpMaxPreGroundedTime;

    [Tooltip("How long after leaving the ground you can trigger a jump (in seconds).")]
    [SerializeField] private float _jumpMaxPostGroundedTime;

    [Space(15f)]
    [Tooltip("The maximum flying speed.")]
    [SerializeField] private float _maxFlySpeed = 10f;

    [Tooltip("Deceleration when flying and not applying acceleration.")]
    [SerializeField] private float _brakingDecelerationFlying;

    [Tooltip("Friction to apply to movement when flying.")]
    [SerializeField] private float _flyingFriction = 1f;

    [Space(15f)]
    [Tooltip("The maximum swimming speed.")]
    [SerializeField] private float _maxSwimSpeed = 3f;

    [Tooltip("Deceleration when swimming and not applying acceleration.")]
    [SerializeField] private float _brakingDecelerationSwimming;

    [Tooltip("Friction to apply to movement when swimming.")]
    [SerializeField] private float _swimmingFriction;
    ...
    [SerializeField] private float _pushForceScale = 1f;

    [Tooltip("Force applied to rigidbodies we stand on (due to mass and gravity) is scaled by this amount.")]
    [SerializeField] private float _standingDownwardForceScale = 1f;
    ...
    [Tooltip("The Cinemachine Virtual Camera for first person view")]
    ...
    private float _crouchedCameraHeight = 1f;
    private float _uncrouchedCameraHeight = 1.65f;
    [Tooltip("Duration of the crouch/uncrouch camera animation")]
    private float _crouchTransitionDuration = 0.2f;
    private float _cameraPitch;

    // Add field to store active tween
    private Tween _crouchTween;

    [Header("Slope Speed Modification")]
    [Tooltip("Should character speed be modified based on slope angle?")]
    public bool enableSlopeSpeedModification;

    [Tooltip("Maximum speed multiplier when moving downhill (1.0 means no speed increase)")]
    [Range(1.0f, 3.0f)]
    public float maxDownhillSpeedMultiplier = 1.5f;

    [Tooltip("Minimum speed multiplier when moving uphill (1.0 means no speed decrease)")]
    [Range(0.0f, 1.0f)]
    public float minUphillSpeedMultiplier = 0.5f;
    ...
    {
      // Stop any active crouch animation
      _crouchTween.Stop();

      // Smoothly adjust camera height using PrimeTween
      if (cameraTarget != null)
      {
        Vector3 targetPos = cameraTarget.transform.localPosition;
        _crouchTween = Tween.LocalPosition(
            cameraTarget.transform,
            new Vector3(targetPos.x, _crouchedCameraHeight, targetPos.z),
    ...
    protected virtual void OnUnCrouched()
    {
      ...
      if (cameraTarget != null)
      {
        ...
        _crouchTween = Tween.LocalPosition(
            cameraTarget.transform,
            new Vector3(targetPos.x, _uncrouchedCameraHeight, targetPos.z),
            _crouchTransitionDuration,
            Ease.OutQuad);
      }
    ...
    public virtual float GetMaxSpeed()
    {
      ...
      {
        case MovementMode.Walking:
          maxSpeed = IsCrouched() ? maxWalkSpeedCrouched : maxWalkSpeed;
          break;
      ...
      // Apply slope speed modification if enabled
      if (enableSlopeSpeedModification && _movementMode == MovementMode.Walking)
      {
        ...
        {
          // Uphill - reduce speed
          float t = Mathf.InverseLerp(0.0f, 90.0f, slopeAngle);
          speedModifier = Mathf.Lerp(1.0f, minUphillSpeedMultiplier, t);
        }
        else
        {
          // Downhill - increase speed
          float t = Mathf.InverseLerp(0.0f, -90.0f, slopeAngle);
          speedModifier = Mathf.Lerp(1.0f, maxDownhillSpeedMultiplier, t);
        }
    ...
    /// </summary>
    public virtual void AddControlPitchInput(float value)
    {
      if (value == 0.0f || cameraTarget == null)
        return;
